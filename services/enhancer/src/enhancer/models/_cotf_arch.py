"""Вендоренная 1:1 архитектура CoTF (CoNet, 3D-LUT exposure correction). Внешний код, ruff/mypy не проверяют."""
...
def pad_tensor(input, divide=16):
    ...
    if width_org % divide != 0 or height_org % divide != 0:
        ...
        width_div = divide - width_res if width_res != 0 else 0
        pad_left = int(width_div / 2) if width_res != 0 else 0
        pad_right = int(width_div - pad_left) if width_res != 0 else 0
        height_div = divide - height_res if height_res != 0 else 0
        pad_top = int(height_div / 2) if height_res != 0 else 0
        pad_bottom = int(height_div - pad_top) if height_res != 0 else 0
