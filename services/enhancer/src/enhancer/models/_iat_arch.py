"""Вендоренная 1:1 архитектура IAT (Illumination-Adaptive-Transformer).

Источник: https://github.com/cuiziteng/Illumination-Adaptive-Transformer
(IAT_enhance/model/{IAT_main,blocks,global_net}.py). Импорты model.* сведены в этот файл,
неиспользуемые/битые timm-импорты убраны. Логику моделей не меняли.
"""
...
class CBlock_ln(nn.Module):
    def __init__(self, dim, mlp_ratio=4.0, qkv_bias=False, qk_scale=None, drop=0.0, attn_drop=0.0,
                 drop_path=0.0, act_layer=nn.GELU, norm_layer=Aff_channel, init_values=1e-4):
        super().__init__()
